...
implicit val barProd = new Equivalence[Bar] {
    type B = HOne
    def extract(a: Bar) = HOne()
    def compose(b: HOne) = Bar()
}

implicit val bazProd = new Equivalence[Baz] {
    type B = HTimes[Int, HTimes[Int, HOne]]
    def extract(a: Baz) = HTimes(a.x, HTimes(a.y, HOne()))
    def compose(b: B) = b match { case HTimes(x, HTimes(y, HOne())) =>
        Baz(x, y)
    }
}
