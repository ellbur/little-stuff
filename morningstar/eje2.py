...
def messup(path, i, beat):
	...
	messed = work_around(path, i, okj, beat)
	if messed == None:
		return path
	
	return messed

def work_around(path, i, okj, beat):
	
	if i == 0 and okj == 0:
		return None
	
	up_fix = work_around_up(path, i, i, okj, beat, okj)
...
def work_around_up(path, i, fi, fj, beat, okj):
	
	work = [ j for j in path ]
	work[i] = okj
	
	if i == 0 and okj != 0:
		return work_around_dn(path, fi, beat, okj)
	...
	for k in range(len(agenda)):
		...
		work_better = work_around_up(work, i-1, fi, fj, beat, agenda[k])
		if work_better != None:
			return work_better
		else:
			pass
...
def work_around_dn(path, i, beat, okj):
	...
	for k in range(len(agenda)):
		...
		work_better = work_around_dn(work, i+1, beat, agenda[k])
		if work_better != None:
			return work_better
...
def improve_by_tweaking(path):
	...
	for i in range(1, R-1):
		...
		messed = messup(path, i, assess(path))
